	...
	function baseNode(config) {
		...
		node.inputFunction=function(msg) {
			...
			try{
				...
				if(!node.hubNode.hub)  throw Error("hub "+node.hubNode.name+" ("+node.hubNode.id+") found  but not PubSubHub");
				...
				if(logger.active) logger.send({label:"inputFunction failed",error:ex.message,stack:ex.stack});
